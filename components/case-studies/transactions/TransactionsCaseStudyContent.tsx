...
const COPY = {
  en: {
    ...
    moneyPathP2: "One more layer of complexity: the actual payment processing was handled by a third-party provider called Cobre. Once a payment entered the processing stage, Payana lost visibility \u2014 we had to wait for Cobre\u2019s response to know if it went through or got rejected. That lag made the money path even more critical to design well.\n\nIt also meant that getting the beneficiary data right upfront was essential. If the recipient\u2019s bank details were wrong, the payment would fail at the Cobre level \u2014 and fixing it meant the user had to correct the data and reprocess. That\u2019s why the rejection flow wasn\u2019t just about showing an error \u2014 it was about making the correction and retry seamless, right from the same screen.",
  ...
  es: {
    ...
    moneyPathP2: "Una capa m\u00e1s de complejidad: el procesamiento real de los pagos lo manejaba un proveedor externo llamado Cobre. Una vez que un pago entraba en la etapa de procesamiento, Payana perd\u00eda visibilidad \u2014 hab\u00eda que esperar la respuesta de Cobre para saber si pas\u00f3 o fue rechazado. Ese lag hizo que el recorrido del dinero fuera a\u00fan m\u00e1s cr\u00edtico de dise\u00f1ar bien.\n\nTambi\u00e9n signific\u00f3 que tener los datos del beneficiario correctos desde el principio era esencial. Si los datos bancarios del destinatario estaban mal, el pago fallaba a nivel Cobre \u2014 y corregirlo implicaba que el usuario ten\u00eda que editar los datos y reprocesar. Por eso el flujo de rechazo no era solo mostrar un error \u2014 era hacer que la correcci\u00f3n y el reintento fueran fluidos, desde la misma pantalla.",
